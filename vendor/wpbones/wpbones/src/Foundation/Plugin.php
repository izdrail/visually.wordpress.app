...
namespace Visually\WPBones\Foundation;

use Illuminate\Database\Capsule\Manager as Capsule;
use Visually\WPBones\Container\Container;
use Visually\WPBones\Contracts\Foundation\Plugin as PluginContract;
use Visually\WPBones\Database\WordPressOption;
use Visually\WPBones\Foundation\Http\Request;
use Visually\WPBones\Foundation\Log\LogServiceProvider;
use Visually\WPBones\Support\Str;
use Visually\WPBones\View\View;
...
class Plugin extends Container implements PluginContract
{
    ...
    public function boot()
    {
         ...
         *
          ...
          Description = "Visually is a WP Bones boilperate plugin"
        ...
        // Load text domain
        load_plugin_textdomain("visually", false, trailingslashit(basename($this->basePath)) . $this->pluginData['DomainPath']);
                    ...
                    if (isset($subMenu['route']['load'])) {
                        ...
                        add_action("load-{$subMenuHook}",
                            function () use ($controller, $method) {
                                $className = "Visually\\Http\\Controllers\\{$controller}";
                                $instance  = new $className;
